...
func main() {
  ...
  games := strings.FieldsFunc(Test, func(r rune) bool { 
    if r == '\n' { return true } 
    return false 
  })
  ...
  for i, game := range games {
    cut := false
    color := ""
    for setIndex, set := range game {
      //cut the begining Game x:
      if string(set) == ":" { 
        cut = true
        continue
      }
      ...
      // add numbers
      num, err := strconv.Atoi(string(set))
      if err == nil {
      ...
      //map the games
      if set == ',' || set == ';' || setIndex == len(game) - 1 {
        if setIndex == len(game) - 1 { color += string(set) }
        if len(count) == 1 {
          gameResult[color] = count[0]
        } else {
          gameResult[color] = count[0] * 10 + count[1]
        }
        count = nil
        color = ""
      } else {
      ...
      if set == ';' || setIndex == len(game) - 1 {
        for k, v := range gameResult {
          if v > bag[k] && !slices.Contains(result, i + 1) { 
