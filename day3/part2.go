...
func entireNumber(line string, index int) (int, int) {
  var num string 
  var start, end int

  for i := index; i >= 0; i-- {
    if line[i] == '.' || !strings.Contains(NUMBERS, string(line[i])) && line[i] != '.' {
      start = i + 1
      break
    }
  }
...
func main() {
  ...
  var partNums []int
  var symbolList, gearList [][]int

  lines := strings.Fields(test)
  //identify * locations
  for lineDex, line := range lines {
    for charDex, char := range line {
      if char == '*' {
        symbolList = append(symbolList, []int{lineDex, charDex})
      }
    }
  }
  //indentify gears from *s
  for _, symbolLocation := range symbolList {
    count = 0
    for i := -1; i <= 1; i++ {
      endOfNumber := -1
      for charDex, char := range lines[symbolLocation[0] + i] {
        if endOfNumber >= charDex { continue }
        if strings.Contains(NUMBERS, string(char)) && charDex >= symbolLocation[1] - 1 && charDex <= symbolLocation[1] + 1 {
          _, end := entireNumber(lines[symbolLocation[0] + i], charDex)
          endOfNumber = end
          count += 1
        }
      }
    }
    if count == 2 { gearList = append(gearList, symbolLocation) }
  }

  //find the numbers touching gears
  for _, gearLocation := range gearList {
    for i := -1; i <= 1; i++ {
      endOfNumber := -1
      for charDex, char := range lines[gearLocation[0] + i] {
        if endOfNumber >= charDex { continue }
        if strings.Contains(NUMBERS, string(char)) && charDex >= gearLocation[1] - 1 && charDex <= gearLocation[1] + 1 {
          wholeNumber, end := entireNumber(lines[gearLocation[0] + i], charDex)
          endOfNumber = end
          partNums = append(partNums, wholeNumber) 
        }
      }
    }
  }
  for dex, num := range partNums {
    if dex == 0 || dex % 2 == 0 { continue }

    result += partNums[dex - 1] * num
  }
  fmt.Println(result)
}
